...
- **New-resource UID declaration.** A newly-authored `.tres` must declare `uid="uid://..."` at the top, and every new `[ext_resource type="Script"]` line in a `.tscn`/`.tres` must carry `uid=`. Flag a new resource that ships missing it (style-warden is `.gd`-only and never sees this). Source: the code-style skill.
