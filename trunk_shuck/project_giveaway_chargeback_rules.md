...
description: no open key sales from the studio's own store; no bulk key issuance outside Steam's curated key system; per-band ceilings on giveaway volume; quarterly G2A audits
type: project
parent: trunk_shuck
...
- **No open key sales from the studio's own storefront.** Keys are issued via Steam's curated key system only (Keymailer, Lurkit, manual curator approvals, Humble / Yogscast bundle inclusions when accepted). The studio never operates a "buy a key directly from us" surface; that surface is the card-fraud attack vector.
- **No bulk key issuance to anyone outside Steam's curated key system.** Same reason.
- **Per-band giveaway ceilings** (full table at `designs/research/giveaway-strategy.md`): about 2,975 keys at Band 1, 4,465 at Band 2, 4,025 at Band 3 floor. These are floor-based *upper bounds*; at the £500K cap (top of Band 3) the effective ceiling climbs to about 26,830. The recommended operating budget is **a quarter to a third of the ceiling**.
...
- **Never commit to open-ended matching of a community fundraising total** (also in `project_marketing_public_good.md`). Cap pledges at a fixed amount per band.
...
- When proposing a "buy a key directly from us" surface for any reason, decline. Direct from Steam, direct from itch, or via a curated reseller (Humble Store on the studio's behalf) only.
- When designing community giveaway mechanics, prefer pledge-cap models over volume-uncapped models.
- Don't lift the per-band ceilings without an updated workup that confirms the cannibalisation prior holds.
