...
References: the branch name carries the GitHub `#N` and drives the Linear issue link. Public surfaces stay GitHub-facing, so Linear `SH-N` stays private (`feedback_design_docs_subject_first_github_ids`). Why the branch is the number's only home: a `closes #N` verb hands GitHub the issue-close on merge, carrying the Linear issue to Closed against the manual-merge intent, and keeping every number in the branch makes that form one nobody reaches for.
