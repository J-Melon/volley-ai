...
- The full structure / map. So they know what's around them and don't restate material that lives elsewhere.
...
| Vocabulary | docs-and-writing (grep-driven dispatch) | Banned phrases and stale terms across the diff. `register`-as-tone, `cozy` self-descriptor, era→acts, etc. |
| Repetition | repetition-reviewer | Does this restate material that lives elsewhere? Cross-doc duplication finder. |
...
| Cross-ref hygiene | docs-and-writing | Links resolve. Each link points at the home doc for what it's referencing. No dead refs to renamed or deleted files. |

Each reviewer follows `../reviewers/SKILL.md` for verdict shape. Approves are silent label-only; blocks post inline review comments anchored to `path:line`, never on the main PR thread.
...
- **Trim-and-verify before push.** Every diff that removes material names where the material now lives. The dispatcher checks before staging.
- **The plan is the contract.** If the work reveals the plan is wrong, escalate to Josh and revise the plan. Don't drift the work.
- **Phase folders are not settled design.** If the work surfaces phase-folder material that reads as settled design, the move is to promote it to the right discipline folder, not to polish it in place.
