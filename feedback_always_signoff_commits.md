...
---
Every commit on every branch must include a `Signed-off-by:` trailer matching `git config user.name` (`Josh`) and email `josh@shuck.gg`. This applies to commits I author directly AND to every code-writing agent dispatched into a worktree.
