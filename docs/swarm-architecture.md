...
How Volley's parallel agent system is shaped, why it is shaped that way, and where the open edges still are. Reference material (role rosters, commit templates, tier table) lives in `ai/swarm/README.md` (game repo); the protocol material lives in [`.claude/skills/dispatch/SKILL.md`](../skills/dispatch/SKILL.md), [`.claude/skills/commits/SKILL.md`](../skills/commits/SKILL.md), and [`.claude/skills/reviewers/SKILL.md`](../skills/reviewers/SKILL.md). What's in flight reads off Linear's `Dispatched` state and `gh pr list`, not a tracked board. This doc is the design layer above all of them: it opens with the lifecycle, the five stages a mission runs in order, then the rationale for why the system is shaped this way.
