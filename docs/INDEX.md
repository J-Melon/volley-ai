...
How the agent system that helps build Volley! is shaped, and what it's allowed to do. The runtime reference (role rosters, commit templates, tier table) lives in `ai/swarm/README.md` (game repo); the protocol material lives in [`.claude/skills/dispatch/SKILL.md`](../skills/dispatch/SKILL.md), [`.claude/skills/commits/SKILL.md`](../skills/commits/SKILL.md), and [`.claude/skills/reviewers/SKILL.md`](../skills/reviewers/SKILL.md). What's in flight reads off Linear's `Dispatched` state and `gh pr list`, not a tracked board. This folder is the design layer above all of those.
